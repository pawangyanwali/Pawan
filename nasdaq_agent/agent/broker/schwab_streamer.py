...
def _process_levelone_equities(content: list) -> None:
    ...
    with _lock:
        for item in content:
            ...
            quote["ws_updated_at"] = quote["updated_at"]
            ...
            _pending_ws_prices[sym] = {
                ...
                "ws_updated_at": quote["ws_updated_at"],
                "rest_updated_at": float(quote.get("rest_updated_at") or 0.0),
                "source": "SCHWAB_WS",
                "source_status": "LIVE",
                "is_live": True,
            }
...
def _process_message(raw: str) -> None:
    ...
    for resp in msg.get("response", []):
        ...
        tracked = None
        with _lock:
            tracked = _ws_subscription_requests.pop(request_id, None)
        if code == 0:
            if svc == "LEVELONE_EQUITIES" and tracked:
                tracked_cmd, symbols, _ = tracked
                with _lock:
                    if tracked_cmd == "UNSUBS":
                        _ws_acknowledged_tickers.difference_update(symbols)
                    else:
                        _ws_acknowledged_tickers.update(symbols)
    ...
    while True:
        try:
            ...
            async with websockets.connect(ws_url, ping_interval=20, ping_timeout=30) as ws:
                ...
                _ws_error = None
                with _lock:
                    _ws_active_tickers.clear()
                    _ws_acknowledged_tickers.clear()
                    _ws_subscription_requests.clear()
                ...
                await _reconcile_equity_subscriptions(
                    ws,
                ...
                logger.info(f"[Streamer] Requested {len(_ws_desired_tickers)} equities, "
                ...
                async def _subscription_reconciler():
                    while True:
                        ...
                        try:
                            ...
                            logger.warning(
                                "[Streamer] Subscription reconciliation failed: %s",
                                exc,
                            )

                reconcile_task = asyncio.create_task(_subscription_reconciler())
...
def update_md_poller_tickers(tickers: list[str]) -> None:
    ...
    global _mdpoller_tickers
    normalized = list(dict.fromkeys(str(t).upper() for t in tickers if t))
    with _lock:
        _mdpoller_tickers = normalized
    logger.info("[MDPoller] Universe updated to %d tickers", len(normalized))


def update_streamer_tickers(tickers: list[str]) -> None:
    """Hot-reload the desired WS universe; the socket reconciles within 2s."""
    global _ws_desired_tickers
    normalized = list(dict.fromkeys(str(t).upper() for t in tickers if t))
    with _lock:
        _ws_desired_tickers = normalized
    logger.info("[Streamer] Desired universe updated to %d tickers", len(normalized))
...
def start_md_poller(tickers: list[str], interval: float = 1.0,
    ...
    """
    global _mdpoller_thread, _mdpoller_tickers
    ...
    def _process_and_broadcast(quotes: dict) -> bool:
        ...
        with _lock:
            for sym, q in quotes.items():
                ...
                now = time.time()
                rest_volume = max(0.0, float(q.get("volume") or 0.0))
                ws_updated_at = float(
                    quote.get("ws_updated_at")
                    or (
                ...
                if (
                    ...
                    and ws_updated_at > 0
                    and now - ws_updated_at <= _WS_QUOTE_PRIORITY_TTL_S
                ):
                    # REST carries an authoritative cumulative-volume snapshot.
                    # Feed it into the shared bar builder even while WS remains
                    # the effective executable price source.
                    if rest_volume > 0:
                        bar_quote = dict(quote)
                        bar_quote["volume"] = rest_volume
                        bar_quote["last"] = float(q.get("last") or quote.get("last") or 0.0)
                        completed = _accumulate_one_minute_bar_locked(sym, bar_quote)
                        if completed:
                            completed_bars.append((sym, completed))
                    quote["volume"] = max(float(quote.get("volume") or 0.0), rest_volume)
                    quote["rest_updated_at"] = now
                ...
                bulk[sym] = {
                    ...
                    "ws_updated_at": float(quote.get("ws_updated_at") or 0.0),
                    "rest_updated_at": quote["rest_updated_at"],
                    "source": "SCHWAB_REST",
    ...
    def _poll_loop() -> None:
        ...
        while True:
            ...
            try:
                ...
                with _lock:
                    cycle_tickers = list(_mdpoller_tickers)
...
def start_streamer(tickers: list[str]) -> None:
    ...
    """
    global _streamer_thread, _event_loop, _ws_desired_tickers

    with _streamer_start_lock:
        if _streamer_thread and _streamer_thread.is_alive():
            update_streamer_tickers(tickers)
        ...
        _ws_desired_tickers = list(dict.fromkeys(str(t).upper() for t in tickers if t))
        _ensure_bar_persist_worker()
        ...
        def _run():
            ...
            loop.run_until_complete(_streamer_main())
...
def ws_fresh_coverage(max_age_s: float = 2.0) -> float:
    """Return fraction of desired tickers with a recent Schwab WS event."""
    with _lock:
        tickers = list(_ws_desired_tickers)
        seen_at = dict(_ws_seen_at)
    if not tickers:
        return 0.0
    now = time.time()
    fresh = sum(now - float(seen_at.get(ticker) or 0.0) <= max_age_s for ticker in tickers)
...
def get_streamer_status() -> dict:
    ...
    with _lock:
        ...
        active_count = len(_ws_active_tickers & desired)
        acknowledged_count = len(_ws_acknowledged_tickers & desired)
        pending_count = len(_ws_subscription_requests)
        seen_count = sum(ticker in _ws_seen_at for ticker in _ws_desired_tickers)
        now = time.time()
        fresh_count = sum(
            now - float(_ws_seen_at.get(ticker) or 0.0) <= 2.0
            for ticker in _ws_desired_tickers
        )
        active_60s_count = sum(
            now - float(_ws_seen_at.get(ticker) or 0.0) <= 60.0
            for ticker in _ws_desired_tickers
        )
        rest_count = sum(
            str((_live_quotes.get(ticker) or {}).get("source_status") or "").upper()
            == "REST_FALLBACK"
            for ticker in _mdpoller_tickers
        )
        candle_count = len(_live_candles)
        halted_count = len(_halted)
        last_ws_age_s = (
            round(now - _last_ws_data_at, 3) if _last_ws_data_at > 0 else None
        )
    ...
    return {
        ...
        "ws_streamer": {
            ...
            "sent_subscriptions": active_count,
            "acknowledged_subscriptions": acknowledged_count,
            "pending_subscription_requests": pending_count,
            "subscription_coverage_pct": round(
                acknowledged_count / desired_count * 100.0, 1
            ) if desired_count else 0.0,
            "seen_quotes": seen_count,
            "active_quotes_60s": active_60s_count,
            "live_quotes": fresh_count if (_streamer_thread and _streamer_thread.is_alive()) else 0,
            "fresh_coverage_pct": round(ws_fresh_coverage(max_age_s=2.0) * 100, 1),
            "last_data_age_s": last_ws_age_s,
            "live_candles":   candle_count,
            "halted_tickers": halted_count,
            "nq_bias":   get_nq_futures_bias(),
        ...
        "md_poller": {
            ...
            "universe_size": len(_mdpoller_tickers),
            "fallback_quotes": rest_count,
            "live_quotes":  rest_count,
